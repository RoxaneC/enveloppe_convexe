...
function chemin = chemin_convexe_haut(p1, p2, M)
  ...
  if hors_connexe == 0
    % s'il n'y a aucun point au dessus du segment, renvoie simplement le segment [p1;p2]
    chemin = [p1;p2];
  elseif hors_connexe == 1
    % s'il n'y a qu'un point au dessus du segment, on l'ajoute directement, puis
    ...
    % s'il y a plusieurs points au dessus du segment, on stocke leurs coordonnées
